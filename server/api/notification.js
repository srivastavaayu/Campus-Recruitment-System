...
router.post("/notifyData",authentication,async (req,res)=>{
    ...
    if(creatorRole == "admin"){
        const {students,placementCoordinators,companies,title,message} = req.body;
        if(!title || !message){
            return res.status(422).json({error : "Plz filled the fields properly"});
        }
        try{

            const notify = new Notification({students,placementCoordinators,companies,title,message,creatorUserName,creatorRole});
        ...
        try{
            ...
            const notify = new Notification({students,companies,title,message,creatorUserName,creatorRole});
...
router.get('/getNotification',authentication,async (req,res)=>{
    ...
    if(role=="student"){
        ...
        notification = await  Notification.find({ $or: [ {"placementCoordinators":true}, {creatorRole:"placement"  }]});
